...
- **Cache** — `actor Cache` backed by App Group `UserDefaults` (`group.ink.codes.Patrol`), shared between app and widget. Methods are `throws` (not `async`) — actor isolation handles concurrency. Stores `SurfEntry` values keyed by `Place.id`, and the selected place as a bare id (the plugin stays the source of truth for its display name). `selectedConditions()` exists for read-only consumers like the iOS app, which has no plugins linked and so can't resolve a `Place` itself. `settings<T: PlaceSettings>(for:)` / `setSettings(_:for:)` store per-place preference structs that opt in via the `PlaceSettings` marker protocol, keyed by place id **and** `T`'s type name so unrelated settings types don't collide — Core defines no concrete settings struct itself; each consumer (watch app, widget) owns its own and declares conformance to store it.
